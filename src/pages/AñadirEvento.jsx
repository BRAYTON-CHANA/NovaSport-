...
function AñadirEvento() {
  const { database, agregarEventoCompleto } = useFormulario(); 
  ...
  const getInitialFormState = (db, currentSettings) => ({
    nombreEvento: 'Evento de Prueba',
    instalacion: (db.Instalaciones && db.Instalaciones[0]?.nombre) || '',
    tipoEvento: (currentSettings.comboValues.tiposEvento && currentSettings.comboValues.tiposEvento[0]) || '',
    motivo: (currentSettings.comboValues.motivos && currentSettings.comboValues.motivos[0]) || '',
    descripcion: '',
    nombreRemitente: 'Juan Pérez',
    cargo: 'Organizador',
    representaInstitucion: false,
    cualInstitucion: '',
    nombreSolicitante: 'Maria Lopez',
    dniRuc: '12345678',
    correo: 'maria.lopez@email.com',
    telefono: '987654321'
  });

  const [formData, setFormData] = useState(database ? getInitialFormState(database, settings) : {});
  const [horarios, setHorarios] = useState([]);
  const [errors, setErrors] = useState({});

  useEffect(() => {
    if (database) {
      setFormData(getInitialFormState(database, settings));
    }
  }, [database, settings]);
  ...
  const handleFinalSubmit = async () => {
    ...
    const eventoData = { ...formData };
    const horariosData = horarios.map(({id, ...resto}) => resto);

    await agregarEventoCompleto({ eventoData, horariosData });
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
        ...
        <main className="grow"><div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto"><h1 className="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold mb-8">Añadir Nuevo Evento</h1><div className="bg-white dark:bg-gray-800 shadow-lg rounded-sm border border-gray-200 dark:border-gray-700 p-6"><form onSubmit={e => e.preventDefault()}>
          ...
          <div className="flex justify-between items-center mt-8">
            ...
            <div className="flex gap-4">
                ...
                {step === 2 && <button onClick={handleFinalSubmit} className="btn bg-emerald-500 hover:bg-emerald-600 text-white">Guardar Evento</button>}
            </div>
          </div>
        </form></div></div></main>
